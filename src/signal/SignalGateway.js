...
export class SignalServerStore {
    ...
    async registerNewPreKeyBundle(userId, preKeyBundle) {
        ...
        const { data } = await Axios.post("http://192.168.1.12:8080/bundlekey/save", { bundleKey:storageBundle, "userID":userId });
console.log("here"+data)
    ...
    getPreKeyBundle(userId) {
        ...
        //let preKey = preKeyBundle.prekeys
    ...
    getPreKeyBundlefromserv(preKeyBundle) {
    let preKey = preKeyBundle.preKeys.splice(-1)

        console.log(preKeyBundle)
        //let preKey = preKeyBundle.prekeys
        preKey[0].publicKey = util.base64ToArrayBuffer(preKey[0].publicKey)
        preKey[0].keyId=parseInt(preKey[0].keyId)
       // this.updatePreKeyBundle(userId, preKeyBundle)
        return {
            identityKey: util.base64ToArrayBuffer(preKeyBundle.identityKey),
            registrationId: parseInt(preKeyBundle.registrationId),
            signedPreKey: {
                keyId: parseInt(preKeyBundle.signedPreKey.keyId),
                publicKey: util.base64ToArrayBuffer(preKeyBundle.signedPreKey.publicKey),
                signature: util.base64ToArrayBuffer(preKeyBundle.signedPreKey.signature),
            },
            preKey: preKey[0]
...
console.log(remoteUserId)
        /* await Axios.get("http://192.168.1.12:8080/bundlekey/get/"+remoteUserId).then(async(x)=>
         
         {               console.log(x.data.data.bundleKey)
     

             var data=this.signalServerStore.getPreKeyBundlefromserv(x.data.data.bundleKey)
             //var data = this.signalServerStore.getPreKeyBundle(remoteUserId);
            console.log(x.data.data.bundleKey)
            console.log(data)
         await sessionBuilder.processPreKey(data).then(async()=>{
         ...
         //console.log(data.data.bundleKey);
        //await sessionBuilder.processPreKey(x.data.data.bundleKey).then(()=>{
                                            //var sessionCipher = new libsignal.SessionCipher(this.store, address);
                                            //this.store.storeSessionCipher(remoteUserId, sessionCipher);
          //                              }) 
          //  });
*/
         ...
         //var remoteUserPreKey = this.signalServerStore.getPreKeyBundle(remoteUserId);
         var {data} = await Axios.get("http://192.168.1.12:8080/bundlekey/get/"+remoteUserId);
         var remoteUserPreKey=data.data.bundleKey
         remoteUserPreKey = this.signalServerStore.getPreKeyBundlefromserv(remoteUserPreKey);
       //  var remoteUserPreKey2=this.signalServerStore.getPreKeyBundle(remoteUserId);
         // Process a prekey fetched from the server. Returns a promise that resolves
         // once a session is created and saved in the store, or rejects if the
         // identityKey differs from a previously seen identity for this address.
         await sessionBuilder.processPreKey(remoteUserPreKey);
